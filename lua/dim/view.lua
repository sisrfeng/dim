...
function M.enable()
  if not M.enabled then
    ...
    vim.cmd([[
        augroup dim
          autocmd!
          autocmd BufWritePost,CursorMoved,CursorMovedI,WinScrolled * lua require("dim.view").update()
          autocmd WinEnter * lua require("dim.view").on_win_enter()
          autocmd BufWritePost * lua vim.defer_fn(function()require("dim.view").update()end, 0)
          autocmd ColorScheme * lua require("dim.config").colors()
        augroup end]])
    M.started = true
    M.on_win_enter()
  end
...
function M.disable()
  if M.enabled then
    M.enabled = false
    pcall(vim.cmd, "autocmd! dim")
    pcall(vim.cmd, "augroup! dim")
...
function M.dim(buf, lnum)
  ...
  -- vim.api.nvim_buf_add_highlight(buf, ns, "Dim", lnum, 0, -1)
end
